...
Element * Element::get(int key_) {
    ...
    if (key_ == _key) { return this; }
    if (key_ < _key) { return _left->get(key_); }
    return _right->get(key_);
}
...
void testGetReturnsCorrectElement(Element * e, int key, bool keyContained) {
    ...
    if (!keyContained) {
        ...
        else if (key != result->_key) {
            cout << endl << "FEHLER: " << key << "erwartet get() liefert aber " << result->_key << ".";
        }
        else {
            cout << "OK";
        }
