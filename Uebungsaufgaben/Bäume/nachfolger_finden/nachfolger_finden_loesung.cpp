...
/*** AUFGABENSTELLUNG:
 * In der Header-Datei `baum.hpp` ist eine einfache Element-Datenstruktur inkl. einer
...
/*** LOESUNG: ***/
Element * Element::successor(int key) {
    Element * res = get(key);
    if (res == nullptr || res->empty())
    {
        return nullptr;
    }
    // rechtes Element auswaehlen --> da in order Ausgabe gefragt
    res = res->_right;
    // wenn es leer ist existiert kein Nachfolger
    if (res->empty()) { return nullptr; }
    // Ansonsten gehe nach links weiter bis zum Ende, da wir den Nachfolger wollen
    /**
     *
     *         42
            /      \
...
int main() {
...

    {
        ...
        cout << result->_key << endl; // Soll 77 ausgeben.
    }
    {
        Element * result = b->successor(77);
        cout << result->_key << endl; // Soll 85 ausgeben.
    }
    {
